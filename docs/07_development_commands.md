...
npm run backend:quality-eval:local:workflow -- --eval-limit 3
...
- `npm run backend:quality-eval:local:workflow -- --eval-limit 3`: 로컬 품질평가 준비 후 바로 품질평가까지 이어서 실행
...
- 한 번에 준비와 평가를 같이 돌리려면 `npm run backend:quality-eval:local:workflow -- --eval-limit 3` 명령을 사용하면 됩니다.
