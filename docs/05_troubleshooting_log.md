...
# [Issue-07] 비즈몽 신뢰성 검증 체계 부재 → 자동 평가 하네스 구축으로 83.3% 정량 개선

> **핵심 요약:** "잘 되는 것 같다" 수준의 체감 테스트만으로는 포트폴리오에서 신뢰성을 주장하기 어려웠다. 그래서 가짜데이터(시나리오 계정) 기반 자동 평가 하네스를 직접 만들고, 라우팅/검색/답변 품질을 같은 질문 세트로 반복 검증하도록 바꿨다. 그 결과 12개 기준 중 10개 통과(83.3%)까지 올렸고, 실패 2개는 다음 개선 과제로 명확히 분리했다.
...
- **핵심 조치 1 — 자동 평가 하네스 구축:**
  - `backend/src/app/dev/bizmong_eval_cases.py`에 12개 평가 케이스를 정의(라우팅 기대값, 기대 공고, 핵심 키워드 포함).
...
- **핵심 조치 3 — 정량 결과 확보:**
  - 동일 12문항 반복 평가 기준으로 **10/12 통과(83.3%)** 달성.
  - 남은 2개 실패 케이스는 라우팅/정책근거 중 어느 축에서 실패했는지 로그로 추적 가능하게 정리.
...
- **실무 포인트 3 — 실패를 남겨야 다음 개선이 빨라진다:** 100% 통과보다 중요한 것은 실패 케이스를 구조화해 다음 스프린트 과제로 연결하는 것이다.
