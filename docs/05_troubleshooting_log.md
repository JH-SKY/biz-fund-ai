...
# [Issue-004] 기기 간 온보딩 상태 불일치 및 인증 로직 설계 오류 해결
> **한 줄 요약:** 클라이언트 캐시(localStorage)에 의존한 온보딩 판별 로직을 서버 측 데이터베이스(Business DB) 기반 검증으로 수정하여 멀티 디바이스 환경에서의 일관성 확보

### 1. 현상 파악 (Symptom)
- **발생 상황:** PC에서 네이버 로그인을 통해 사업장 등록(온보딩)을 마친 유저가 모바일 기기에서 동일한 계정으로 로그인 시, 이미 등록된 사업장이 있음에도 불구하고 다시 온보딩(사업장 등록) 페이지로 강제 리다이렉트되는 현상 발생.
- **핵심 에러 메시지:** - `Redirecting to /onboarding...` (논리적 오류로 인한 의도치 않은 리다이렉션)
  - 별도의 syntax error는 없으나, `is_new_user` 판단 로직이 `true`로 고정됨으로써 사용자 경험(UX) 저해.
- **사용자 가치:** 이미 정보를 입력한 유저에게 반복적인 데이터 입력을 요구함으로써 서비스 이탈률을 높이고 데이터 정합성을 해칠 우려가 있음.

### 2. 해결을 위한 과정 (Trials & Errors)
- **1차 접근: 클라이언트 상태 검증**
  - **나의 생각:** `localStorage`에 저장된 `isOnboarded` 플래그가 모바일에서 `false`인 것을 확인. 단순히 클라이언트 스토리지 문제로 치부함.
  - **실행 결과:** 모바일 브라우저는 스토리지 공유가 안 되므로 당연히 초기값인 `false`로 동작.
  - **원인 파악:** 서비스의 핵심 상태(온보딩 여부)를 브라우저 로컬 스토리지에만 의존하게 설계한 것이 근본적 문제임을 인지.

- **2차 접근: 백엔드 판별 로직(user.nickname) 분석**
  - **나의 생각:** 서버에서 `is_profile_incomplete`를 체크하고 있으니 백엔드 로직은 안전할 것이라 판단.
  - **실행 결과:** 확인 결과, 온보딩 시 `businesses` 테이블만 업데이트하고 `user.nickname`은 건드리지 않아 해당 값은 항상 `null`이었음.
  - **원인 파악:** **판단 기준(User 테이블)과 실제 데이터 저장소(Business 테이블)의 불일치.** 즉, "닉네임이 없으면 신규 유저"라는 가정이 비즈니스 로직과 맞지 않았음.

### 3. 해결 방안 (Resolution)
- **단기 조치:** 백엔드 인증 서비스 로직에서 유저의 닉네임 유무가 아닌, **실제 DB에 해당 유저 소유의 활성 사업장이 존재하는지**를 쿼리하여 온보딩 여부를 결정하도록 수정.
- **근본 대책:** 인증(Authentication)과 온보딩(Business Logic)의 책임을 명확히 분리. 클라이언트 캐시는 UI 최적화 용도로만 사용하고, 핵심 흐름 제어는 SSOT(Single Source of Truth)인 데이터베이스를 기준으로 수행.
- **적용 코드:**
```python
# auth/repository.py - Exist 쿼리를 통한 최적화된 비즈니스 존재 확인
async def has_active_business(self, user_id: uuid.UUID) -> bool:
    from src.app.domains.business.model import Business
    # COUNT보다 EXISTS가 성능상 유리할 수 있으나, 가독성을 위해 scalar() 활용
...
# auth/service.py - 수정된 온보딩 판별 로직
if is_new:
    should_redirect_to_onboarding = True
else:
    # 닉네임 체크 대신 실제 비즈니스 도메인의 데이터를 확인
    has_biz = await self._repo.has_active_business(user.id)
    should_redirect_to_onboarding = not has_biz
```

### 4. 기술적 회고 및 성장 (Retrospective)
- **성능/효율성 변화:** 불필요한 온보딩 과정 재진입을 막아 DB 중복 데이터 생성을 방지하고, 멀티 디바이스 유저의 서비스 연속성을 확보함.
- **새롭게 배운 개념:** **SSOT(Single Source of Truth, 단일 진실 공급원)** 원칙의 중요성을 체감함. 상태 판단 기준이 흩어져 있을 때 발생하는 Side Effect를 경험하며 도메인 간의 책임 분리(Separation of Concerns)를 재검토하게 됨.
- **실무적 인사이트:** - "로컬에서 잘 돌아간다"는 말은 반쪽짜리 검증이다. 특히 JWT와 클라이언트 스토리지를 사용하는 환경에서는 브라우저 캐시가 버그를 가릴 수 있다는 점을 항상 유의해야 한다.
  - 비즈니스 요구사항(사업장 등록)과 데이터 모델(User/Business 테이블) 사이의 논리적 연결 고리를 설계 단계에서 더 면밀히 검토해야 함을 깨달음.

---
**자기반성 요약:** 판단 기준은 실제 데이터가 저장된 곳과 일치시켜야 하며, 다양한 기기 환경에서의 교차 검증은 필수적이다.