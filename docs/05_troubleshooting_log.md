...
> **핵심 요약:** API 응답의 요약 정보와 실제 파일 다운로드의 신뢰성 문제를 '파일 시그니처 감지'와 '중첩 트랜잭션(SAVEPOINT)'으로 해결하여 RAG 시스템의 데이터 완결성을 높였다.
...
- **핵심 해결책 (파일 시그니처 감지 & Savepoint):**
  1. 파일의 첫 1KB 바이너리를 읽어 실제 형식을 감지하는 **파일 시그니처(Magic Number) 탐지 로직**을 도입했다.
...
def detect_file_type(binary_content: bytes) -> str:
    # 파일 시그니처(첫 바이트 패턴)를 확인하여 실제 형식 감지
...
- **새롭게 배운 점:**
  1. **파일 시그니처(Magic Number):** 파일 탐지는 파일명이 아닌 실제 파일의 시작 바이너리 패턴을 보는 방식을 배웠다.
...
# [Issue-05] Admin 배포 직후 로그인 500 + 다중 404 연쇄 오류 복구
...
# [Issue-06] 정책 공고 AI 동기화 파이프라인 장애 — Supabase Connection Timeout & 데이터 품질 오염
...
- **추가 해결 — 파일 선택 로직 개선:**
  `printFileNm`이 없거나 `.pdf`로 끝나지 않아도 `printFlpthNm` URL이 있으면 무조건 다운로드를 시도하도록 수정했다. 실제 PDF 여부는 파일 시그니처(`%PDF`)로 확인한다. 이로써 파싱 성공률이 ~24%에서 ~57%로 개선됐다.
...
# [Issue-07] 비즈몽 신뢰성 검증 체계 부재 → 자동 평가 하네스 구축 및 15/15 통과

> **핵심 요약:** "잘 되는 것 같다" 수준의 체감 테스트만으로는 포트폴리오에서 신뢰성을 주장하기 어려웠다. 그래서 실제 고객 데이터가 아닌 테스트용 사업자 프로필 7종(BIZ-01~BIZ-07)과 대표 상담 질문 15건으로 자동 평가 하네스를 만들고, 라우팅/검색/답변 품질을 같은 질문 세트로 반복 검증하도록 바꿨다. 초기 12개 기준에서는 10개 통과(83.3%)였고, 실패한 우회 표현·복합 의도 질문을 분석해 Query Rewriting, Multi-intent Search Plan, 의도별 섹션 답변을 보완했다. 이후 평가셋을 15개로 확장해 15/15(100%) 통과 기준으로 정리했다.

### 1. 현상 파악 (Symptom)

- **발생 상황:** 정책 매칭과 비즈몽 상담 기능은 수동 테스트에서 "대체로 동작"했지만, 질문 표현이 조금만 바뀌면 결과 편차가 발생했다.
- **핵심 문제:** 품질을 숫자로 설명할 방법이 없어 "신뢰할 수 있는 서비스인가?"라는 질문에 객관적으로 답하기 어려웠다.
- **사용자 가치 영향:** 취업 포트폴리오/면접에서 "어떻게 검증했는가"를 설명하기 어려워 경쟁력이 떨어지는 상태였다.

### 2. 해결을 위한 과정 (Trials & Errors)

- **1차 시도: 프론트 수동 점검 중심**
  - **가설:** 여러 계정으로 직접 질문하면 품질 판단이 가능할 것이라 생각했다.
  - **결과:** 재현성이 낮았다. 같은 의도를 다른 문장으로 물으면 라우팅/근거/답변 톤이 흔들렸고, 개선 전후 비교도 감에 의존했다.
- **2차 시도: 단일 시나리오 고정 테스트**
  - **가설:** 대표 질문 몇 개만 통과하면 실사용도 문제없을 것이라 판단했다.
  - **결과:** 지역/업종/업력/복합목적 질문(고용+자금)에서 누락 케이스가 드러났다.
- **3차 시도: 실패 케이스를 평가셋에 편입**
  - **가설:** 실패한 질문을 별도 메모로만 남기면 같은 회귀가 반복될 수 있다고 판단했다.
  - **결과:** 생활어 기반 복합 의도, 제조업 설비 교체 자연어 질문, 강원 관광업 자연어 질문을 평가셋에 추가해 개선 여부를 고정 기준으로 확인했다.
- **전환 판단:** "사람이 눈으로 확인"에서 "코드로 반복 검증"으로 검증 방식을 바꿔야 한다고 결론냈다.

### 3. 해결 방안 (Resolution)

- **핵심 조치 1 — 자동 평가 하네스 구축:**
  - `backend/src/app/dev/bizmong_eval_cases.py`에 테스트용 사업자 프로필별 평가 케이스를 정의(라우팅 기대값, 기대 공고, 핵심 키워드 포함).
  - `backend/src/app/scripts/evaluate_bizmong_quality.py`를 만들어 dev-login → 테스트용 사업자 컨텍스트 설정 → 세션 생성 → 질문/응답/근거 수집 → pass/fail 집계까지 자동화.
- **핵심 조치 2 — 서비스 로직 고도화:**
  - RAG 라우팅 기준과 일반상담 라우팅 분기를 정교화해 오분류를 줄임.
  - 우회 표현은 `운영자금`, `고용지원`, `시설자금`처럼 검색 가능한 정책 키워드로 재작성하도록 Query Rewriting을 추가.
  - `각각`, `하나씩`, `두 가지` 같은 표현이 있으면 검색 태스크를 의도별로 나누고, 답변도 섹션 단위로 분리하도록 보강.
  - 정책 매칭 근거가 답변 본문에 드러나도록 응답 구조를 보강해 "왜 이 공고를 추천했는지"를 설명 가능하게 만듦.
  - 실패 시 폴백(안전 응답) 경로를 정리해 상담 응답 공백/붕괴를 방지.
- **핵심 조치 3 — 정량 결과 확보:**
  - 초기 12문항 반복 평가 기준으로 **10/12 통과(83.3%)** 달성.
  - 실패 2개를 분석해 복합 의도·우회 표현 로직을 보완한 뒤 평가셋을 15문항으로 확장.
  - 최종 평가 기준으로 **15/15 통과(100%)**를 달성하고, 이후 회귀 테스트 기준으로 사용.

### 4. 기술적 회고 및 성장 (Retrospective)

- **실무 포인트 1 — "동작"과 "신뢰"는 다르다:** 기능 구현만으로는 부족하고, 재현 가능한 자동 평가 기준이 있어야 신뢰성을 주장할 수 있다.
- **실무 포인트 2 — 평가 데이터셋이 제품을 끌어올린다:** 테스트용 사업자 프로필을 단순 데모 용도가 아니라 회귀 테스트 자산으로 전환하니 개선 속도와 정확도가 동시에 올라갔다.
- **실무 포인트 3 — 실패를 평가셋에 편입해야 회귀를 막는다:** 실패 케이스를 단순 메모로 남기지 않고 자동 평가 문항으로 승격해야 다음 개선이 실제 품질 향상으로 이어진다.
- **포트폴리오 활용도:** "AI 챗봇을 만들었다"가 아니라 **"정책 매칭/상담 품질을 정량 검증하고 개선 루프를 설계했다"**로 설명 가능해, 면접에서 기술적 성숙도를 보여주기 좋은 사례다.
