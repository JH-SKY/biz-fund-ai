...
> **핵심 요약:** "잘 되는 것 같다" 수준의 체감 테스트만으로는 포트폴리오에서 신뢰성을 주장하기 어려웠다. 그래서 가짜데이터(시나리오 계정) 기반 자동 평가 하네스를 직접 만들고, 라우팅/검색/답변 품질을 같은 질문 세트로 반복 검증하도록 바꿨다. 초기 12개 기준에서는 10개 통과(83.3%)였고, 실패한 우회 표현·복합 의도 질문을 분석해 Query Rewriting, Multi-intent Search Plan, 의도별 섹션 답변을 보완했다. 이후 평가셋을 15개로 확장해 15/15(100%) 통과 기준으로 정리했다.
...
- **핵심 조치 1 — 자동 평가 하네스 구축:**
  - `backend/src/app/dev/bizmong_eval_cases.py`에 평가 케이스를 정의(라우팅 기대값, 기대 공고, 핵심 키워드 포함).
  - `backend/src/app/scripts/evaluate_bizmong_quality.py`를 만들어 dev-login → 사업장 컨텍스트 설정 → 세션 생성 → 질문/응답/근거 수집 → pass/fail 집계까지 자동화.
- **핵심 조치 2 — 서비스 로직 고도화:**
...
- **실무 포인트 2 — 평가 데이터셋이 제품을 끌어올린다:** 가짜데이터를 단순 데모 용도가 아니라 회귀 테스트 자산으로 전환하니 개선 속도와 정확도가 동시에 올라갔다.
