...
이 문서는 포트폴리오에서 수치로 제시한 세 가지 주요 실험의 설계·과정·결과를 기록한다.

| # | 실험 | 핵심 수치 | 관련 파일 |
|---|------|-----------|-----------|
| 1 | BizMong 품질 검증 하네스 | 10/12 통과 (83.3%) | `scripts/evaluate_bizmong_quality.py` |
| 2 | 비용 최적화 — GPT 호출 수 및 모델 전환 | 약 15배 절감 | `agents/policy_sync_agent.py`, `agents/biz_mong/` |
| 3 | HR Copilot State 유실 진단 | 78.6 → 91.8점 | [Notion 실험일지 →](https://checker-gambler-0f8.notion.site/3611480769ac8067a741c567dc5cf961) |
...
```
총 12케이스 · rag 10개 / stats 1개 / general_qa 1개
사업자 시나리오: BIZ-01 ~ BIZ-07 (업종·지역·상황별 서로 다른 사업자)
```
...
케이스 3 (우회 표현)과 케이스 10 (복합 목적)이 핵심 난이도 케이스다.
...
시나리오별로 다른 사업자 계정(BIZ-01 ~ BIZ-07)으로 로그인해 실제 사업자 컨텍스트를 주입한다.
...
미통과 2케이스의 공통 패턴은 **표현 편차가 큰 케이스**였다.  
우회 표현이나 복합 목적 질문처럼 의도 추론이 필요한 경우, RAG 검색은 정상 동작했지만 응답의 키워드 구성이 예상과 달랐다.

이 결과가 Hybrid RAG(Vector + Keyword + RRF)의 설계 방향을 검증해줬다.  
단일 벡터 검색만으로는 우회 표현 케이스에서 정책 누락이 발생했을 가능성이 높다.

### 1-6. 이 하네스가 해준 것

비용 최적화(실험 2) 이후 품질이 유지되는지 확인할 수 있었다.  
프롬프트를 수정할 때마다 12개를 다시 돌려 회귀 여부를 체크했다.

**"개선했다"가 아니라 "83.3%가 유지됐다"로 말할 수 있게 됐다.**
...
"비용을 줄였다"가 아니라 **"품질 83.3%를 유지하면서 채팅 비용을 약 15배 줄였다"** 로 말할 수 있게 됐다.

---

## 실험 3 — HR Copilot State 유실 진단 실험

> 이 실험은 HR Copilot 팀 프로젝트에서 진행했으며, 자세한 LangSmith 트레이스와 실험 일지는 Notion에 기록돼 있다.  
> [실험일지 보러가기 →](https://checker-gambler-0f8.notion.site/3611480769ac8067a741c567dc5cf961)

### 3-1. 문제 인식

HR Copilot은 LangGraph로 구성된 면접 질문 생성 에이전트다. Reviewer 노드가 낮은 점수를 줘도 **동일한 낮은 품질의 질문이 반복**됐다. 구조는 정상인데 출력이 개선되지 않는 현상.

### 3-2. 가설 소거 과정

| # | 가설 | 확인 방법 | 결론 |
|---|------|-----------|------|
| H1 | Retry 로직에 문제가 있는가? | retry_count State 값 추적 | ✗ 아님 |
| H2 | Reviewer 채점 기준이 문제인가? | reviewer output 전수 확인 | ✗ 아님 |
| H3 | Prompt 품질이 문제인가? | 프롬프트 변경 후 재실험 | ✗ 아님 |
| **H4** | **State 흐름이 끊기는가?** | **노드간 입출력 State 대조** | **✓ 원인** |

**발견:** Reviewer가 피드백 State를 업데이트해도, 재생성 루프에서 Questioner 노드로 재진입할 때 그 State가 초기화되고 있었다.

LangSmith에서 수백 개 트레이스를 따라가며 노드별 입력·출력을 대조하지 않았다면 발견하지 못했을 버그였다. 로직이 아닌 **데이터 흐름 구조**의 문제였기 때문이다.

### 3-3. 수정 후 결과

피드백 State가 다음 생성 루프로 정상 전달되도록 그래프 구조를 수정했다.

```
수정 전: 78.6점 (동일 루브릭 · 10세션 · 50문항 평균)
수정 후: 91.8점
상승폭: +13.2점
```

프롬프트를 고친 게 아니라 **"Reviewer 피드백이 다음 생성 루프로 전달되지 않는 구조"**를 고쳤다.  
이것이 LangSmith를 "감"이 아닌 "근거"로 쓰는 방법이었다.

### 3-4. 측정 방법

동일 루브릭을 Before/After에 모두 적용했기 때문에 수치 비교가 유효하다.

| 항목 | 설계 |
|------|------|
| 측정 단위 | 질문 품질 5항목 + 평가가이드 5항목 |
| 척도 | 1~5점 × 2(가중치) = 100점 환산 |
| 세션 수 | 10세션, 50문항 |
| 기준 고정 | 동일 루브릭으로 Before/After 비교 |

[루브릭 전체 기준 보기 →](https://checker-gambler-0f8.notion.site/3511480769ac80ff96f6f7d2ed60fdf6)
