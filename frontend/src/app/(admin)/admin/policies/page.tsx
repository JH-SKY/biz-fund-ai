...
function PoliciesContent() {
  ...
  const handleBgTrigger = async (label: string, fn: () => Promise<{ status: string; message?: string }>) => {
    try {
      await fn();
      ...
      <Card className="border-primary-200 bg-primary-50/30">
        ...
        <CardContent className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          ...
          <SyncTriggerButton
            ...
            onClick={() =>
              handleBgTrigger("임베딩", () =>
                embedAll.mutateAsync(undefined).then((r) => ({ status: "ok", message: String(r.embedded) }))
              )
            }
          />
        </CardContent>
