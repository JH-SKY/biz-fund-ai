...
from src.app.domains.auth.model import User # 전유저 대상 공지 발송을 위해 참조 추가
...
class NotificationRepository:
    ...
    async def get_notifications(
        ...
        if business_id is not None:
            ...
            stmt = stmt.where(
                (Notification.business_id == business_id) | (Notification.business_id.is_(None))
            )
    ...
    async def get_notification_by_id(self, noti_id: uuid.UUID) -> Optional[Notification]:
        """알림 단건 조회."""
        stmt = select(Notification).where(Notification.id == noti_id)
        result = await self._session.execute(stmt)
        return result.scalar_one_or_none()

    async def update_notification_read_status(self, noti: Notification, is_read: bool) -> None:
        """개별 알림 읽음 상태 업데이트."""
    ...
    async def bulk_create_system_notification(
        ...
        1. 메커니즘: User 테이블의 모든 활성 유저 ID를 조회함과 동시에 Notification 테이블에 바로 Insert 합니다. [cite: 2026-03-05]
        2. 효율성: Python 레벨에서 루프를 돌지 않고 DB SQL 한 번으로 처리하여 성능을 극대화한 '대량 배송' 방식입니다. [cite: 2026-03-05]
        """
        # 활성 유저(status='ACTIVE') 전체에게 발송하는 서브쿼리 생성
        select_users_stmt = (
            select(
                User.id,           # user_id
                func.cast(None, UUID(as_uuid=True)), # business_id (공통공지이므로 None)
                func.cast(noti_type, String(20)),
                func.cast(title, String(255)),
                func.cast(message, Text),
                func.cast(False, Boolean), # is_read
                func.cast(link_url, Text),
                func.now(),        # created_at
                func.now(),        # updated_at
            )
        ...
        # INSERT INTO ... SELECT 문 실행
        stmt = insert(Notification).from_select(
            ["user_id", "business_id", "type", "title", "message", "is_read", "link_url", "created_at", "updated_at"],
            select_users_stmt
        )
